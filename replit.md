...
- **Pre-flight Check Landing Page**: Primary landing page at route `/` that collects user context (role, industry, company size, team size, goals) in a compact form before assessments. Uses 3-column role grid, dropdown for industry, horizontal pills for revenue/team size, and 2-column checkboxes for goals. Features popular journey examples, clear CTAs, and smooth scrolling to input sections. Saves data to UserProfileContext for personalized recommendations. Replaces the older 3-step wizard as the single entry point to the application.
