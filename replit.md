...
- **Quick Marketing Assessment**: A 10-question assessment across key marketing capabilities with a 5-option scale (0-100), leading to R/E/A/O dimensional analysis. Features live journey suggestions that update in real-time as users answer questions. Includes collapsible Question Status panel (default expanded on desktop, collapsed on mobile) to reduce visual clutter. Optimized 4-column layout provides more horizontal space for questions.
...
- **Tech Stack Evaluation**: Categorization and selection of tools across 6 major categories with weighted scoring based on enterprise vs. SMB tools. Features collapsible category accordions, per-category optimization sliders (0-100%), and compact tool listings with tier badges. Includes category-level "Select All" and "Clear" actions.
...
- **What-If Simulator**: Interactive scenario modeling with adjustable parameters (budget, headcount, tech utilization, process maturity) to simulate impact on aircraft level and REAO dimensions. Refactored into reusable ScenarioSimulatorPanel component with compact and full layout modes for embedding in Results page.
- **Journey Map**: First-class Mapbox-based visualization of marketing functions as cities on a global map. Shows real-time heat maps based on REAO scores, journey connections between weak and strong areas, and city-specific insights. Accessible via dedicated `/journey-map` route in cockpit navigation. Features collapsible sidebars for controls/legend and strategic insights, with map-dominant center layout. Includes error handling for map loading issues and graceful fallback for missing assessment data.
...
- **Pre-flight Check Landing Page**: Primary landing page at route `/` that collects user context (role, industry, company size, team size, goals) in a compact form before assessments. Uses custom DropdownSelect component for role, industry, revenue, and team size inputs with clean single-line collapsed state and expandable menus. Goals remain as 2-column multi-select checkboxes. Features popular journey examples, clear CTAs, and smooth scrolling to input sections. Saves data to UserProfileContext for personalized recommendations. Replaces the older 3-step wizard as the single entry point to the application.
